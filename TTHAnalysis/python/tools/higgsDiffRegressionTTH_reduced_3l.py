...
class HiggsDiffRegressionTTH_reduced_3l(Module):
    def __init__(self,label="_Recl", variations=[], cut_BDT_rTT_score = 0.0, btagDeepCSVveto = 'M', doSystJEC=True):
        ...
        self.model_regression = load_model(os.path.join(os.environ["CMSSW_BASE"], "src/CMGTools/TTHAnalysis/data/regressionMVA/dnn_trained_3l.h5"), custom_objects={'loss_MSEDeltaVar': loss_MSEDeltaVar})#os.p\
    ...
    def buildHadronicTop(self, event, score, alljets, jesLabel):
        ...
        if score>self.cut_BDT_rTT_score:
            ...
            #top1.SetPtEtaPhiM(alljets[j1top].p4().Pt(), alljets[j1top].p4().Eta(), alljets[j1top].p4().Phi(), alljets[j1top].p4().M())
            top1.SetPtEtaPhiM(getattr(alljets[j1top], "pt%s"%jesLabel),alljets[j1top].p4().Eta(), alljets[j1top].p4().Phi(), alljets[j1top].p4().M())
            top2 = ROOT.TLorentzVector(); 
            #top2.SetPtEtaPhiM(alljets[j2top].p4().Pt(), alljets[j2top].p4().Eta(), alljets[j2top].p4().Phi(), alljets[j2top].p4().M())
            top2.SetPtEtaPhiM(getattr(alljets[j2top], "pt%s"%jesLabel),alljets[j2top].p4().Eta(), alljets[j2top].p4().Phi(), alljets[j2top].p4().M())
            top3 = ROOT.TLorentzVector(); 
            #top3.SetPtEtaPhiM(alljets[j3top].p4().Pt(), alljets[j3top].p4().Eta(), alljets[j3top].p4().Phi(), alljets[j3top].p4().M())
            top3.SetPtEtaPhiM(getattr(alljets[j3top], "pt%s"%jesLabel),alljets[j3top].p4().Eta(), alljets[j3top].p4().Phi(), alljets[j3top].p4().M())
    ...
    def analyze(self, event):
        ...
        #if len(leps) < 3                      : return setDefault(event, jesLabel)
        ...
        #if abs(leps[0].charge + leps[1].charge + leps[2].charge) == 0  : return False #cha
        ...
        for jesLabel in self.systsJEC.values():
            ...
            # Temporary fix for very strange bug
            if event._entry == 0 : 
                self.setDefault(event, jesLabel) # pt251510
                continue
            #print "gen jet pt", GenJet_pt[gj]
            ...
            if leps[0].pt < 25 or leps[1].pt < 15 or leps[2].pt < 10 : 
                self.setDefault(event, jesLabel) # pt251510
                continue

            if event.nLepTight_Recl > 3           : 
                self.setDefault(event, jesLabel) # exclusive
                continue

            if abs(event.mZ1_Recl-91.2)<10        : 
                self.setDefault(event, jesLabel) #cha
                continue
            if abs(leps[0].charge + leps[1].charge + leps[2].charge) == 0  : 
                self.setDefault(event, jesLabel) #cha
                continue
            ...
            for j in alljets:
                ...
                #j_temp.SetPtEtaPhiM(getattr(j, "pt%s"%jesLabel), j.eta, j.phi, j.mass )
                jets.append(j)
            # Store all jets
            ...
            if(len(jets) >= 2):
                ...
                self.out.fillBranch('%sreco_q1_q2_score_higgs_pt%s'%(self.label,jesLabel), (j1_q+j2_q).Pt())
