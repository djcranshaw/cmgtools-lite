...
class HiggsDiffRegressionTTH_reduced_3l(Module):
    def __init__(self,label="_Recl", variations=[], cut_BDT_rTT_score = 0.0, btagDeepCSVveto = 'M', doSystJEC=True):
        ...
        self.model_regression = load_model(os.path.join(os.environ["CMSSW_BASE"], "src/CMGTools/TTHAnalysis/data/regressionMVA/dnn_trained_3l.h5"), custom_objects={'loss_MSEDeltaVar': loss_MSEDeltaVar})#os.path.join(os.environ["CMSSW_BASE"], "dnn_trained.h5"))
    ...
    def analyze(self, event):
        ...
        if abs(leps[0].charge + leps[1].charge + leps[2].charge) == 0  : return False #charge convent
